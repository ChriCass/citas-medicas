...
<?php else: ?>
  ...
  <div class="input-with-toggle">
    <input class="input" type="text" name="diagnostico" id="diagnostico_input" value="<?= htmlspecialchars($c['diagnostico_nombre'] ?? ($c->diagnostico?->nombre_enfermedad ?? '')) ?>" autocomplete="on" aria-haspopup="listbox" aria-expanded="false">
    ...
    <input type="hidden" name="diagnostico_id" id="diagnostico_id" value="<?= htmlspecialchars($c['diagnostico_id'] ?? ($c->diagnostico?->id ?? '')) ?>">
  </div>
  ...
  if (recEmpty && hid && hid.value !== '' ){ e.preventDefault(); setFieldErr(document.getElementById('add_receta_btn'),'err_receta','Debes agregar al menos una receta con medicamento, indicación o duración.'); firstInvalid = firstInvalid || document.getElementById('add_receta_btn'); }
    ...
    // Inicializar si ya hay texto
    (function(){ const v = (input.value||'').trim(); if (v.length>0) fetchDiagnosticos(v); })();
